...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-medium text-gray-900">Edit Approval Request</h1>
        </div>
        <a href="{{ route('approvals.show', $approval) }}" class="px-4 py-2 bg-gray-200 hover:bg-gray-200/70 text-gray-700 font-medium rounded-lg transition-colors">
            <i class="fas fa-arrow-left mr-2"></i>Back
        </a>
    </div>

    <!-- Form -->
    <form action="{{ route('approvals.update', $approval) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 gap-6">
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $approval->title) }}" required
                        placeholder="e.g., Homepage Design Approval"
                        class="w-full rounded-lg border-gray-300 focus:border-[#1a425f] focus:ring-[#1a425f] @error('title') border-red-500 @enderror">
                    @error('title')
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4"
                        ...
                        class="w-full rounded-lg border-gray-300 focus:border-[#1a425f] focus:ring-[#1a425f] @error('description') border-red-500 @enderror">{{ old('description', $approval->description) }}</textarea>
                    @error('description')
                    <p class="mt-1  text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="version" id="version" value="{{ old('version', $approval->version ?? 'v1') }}"
                        placeholder="e.g., v1, v2, v2.1"
                        class="w-full rounded-lg border-gray-300 focus:border-[#1a425f] focus:ring-[#1a425f] @error('version') border-red-500 @enderror">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="md:col-span-2">
                    ...
                    <select name="client_id" id="client_id" required
                        class="w-full rounded-lg border-gray-300 focus:border-[#1a425f] focus:ring-[#1a425f] @error('client_id') border-red-500 @enderror">
                        <option value="">Choose a client...</option>
                        @foreach($clients as $client)
                        <option value="{{ $client->id }}" {{ old('client_id', $approval->client_id) == $client->id ? 'selected' : '' }}>
                            {{ $client->full_name }}
                ...
                <div>
                    ...
                    <select name="priority" id="priority" required
                        class="w-full rounded-lg border-gray-300 focus:border-[#1a425f] focus:ring-[#1a425f] @error('priority') border-red-500 @enderror">
                        <option value="low" {{ old('priority', $approval->priority ?? 'medium') == 'low' ? 'selected' : '' }}>Low</option>
                        <option value="medium" {{ old('priority', $approval->priority ?? 'medium') == 'medium' ? 'selected' : '' }}>Medium</option>
                        <option value="high" {{ old('priority', $approval->priority ?? 'medium') == 'high' ? 'selected' : '' }}>High</option>
                    </select>
                    @error('priority')
                    <p class="mt-1  text-red-600">{{ $message }}</p>
                    @enderror
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 gap-6">
                <div>
                    ...
                    <input type="date" name="due_date" id="due_date" value="{{ old('due_date', $approval->due_date?->format('Y-m-d')) }}"
                        min="{{ date('Y-m-d') }}"
                        class="w-full rounded-lg border-gray-300 focus:border-[#1a425f] focus:ring-[#1a425f] @error('due_date') border-red-500 @enderror">
                ...
                <div>
                    ...
                    <textarea name="message" id="message" rows="3"
                        ...
                        class="w-full rounded-lg border-gray-300 focus:border-[#1a425f] focus:ring-[#1a425f] @error('message') border-red-500 @enderror">{{ old('message', $approval->message) }}</textarea>
                    @error('message')
                    <p class="mt-1  text-red-600">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <div>
                ...
                <textarea name="internal_notes" id="internal_notes" rows="3"
                    ...
                    class="w-full rounded-lg border-gray-300 focus:border-[#1a425f] focus:ring-[#1a425f] @error('internal_notes') border-red-500 @enderror">{{ old('internal_notes', $approval->internal_notes) }}</textarea>
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="px-4 py-2 bg-[#1a425f] hover:bg-[#1a425f]/90 text-white font-medium rounded-lg transition-colors shadow-sm">
                <i class="fas fa-save mr-2"></i>Update Approval Request
            </button>
        </div>
    </form>
</div>
...
<div class="max-w-2xl mx-auto mt-8">
    <div class="bg-red-50 border border-red-200 rounded-lg p-8 text-center">
        ...
        <p class="text-red-700 mb-6">You don't have permission to edit this approval request.</p>
